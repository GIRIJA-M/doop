...
function hotelSort(hotel1, hotel2) {
  return hotel1.price - hotel2.price;
}
app.get('/hotels/sort/pricing', (req, res) => {
  let pricing = hotels.slice();
  pricing.sort(hotelSort);
  res.json(pricing);
});

//... 02 ...//
function ratingSort(hotel1, hotel2) {
  return hotel1.rating - hotel2.rating;
}
app.get('/hotels/sort/rating', (req, res) => {
  let rating = hotels.slice();
  rating.sort(ratingSort);
  res.json(rating);
});

//... 03 ...//

function reviewsSort(hotel1, hotel2) {
  return hotel1.reviews - hotel2.reviews;
}
app.get('/hotels/sort/reviews', (req, res) => {
  let review = hotels.slice();
  review.sort(reviewsSort);
  res.json(review);
});
...
function filterByCountry(hote, country) {
  return hote.country === country;
}
app.get('/hotels/filter/country', (req, res) => {
  let country = req.query.country;
  let result = hotels.filter((hote) => filterByCountry(hote, country));
  res.json(result);
});
